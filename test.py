...
ConfigLoader.apply("test")    # ← 必须在框架 import 之前执行
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(dataset):
        ...
        if i % 5 == 0:  # save images to an HTML file
            if i==0:
               print(f"[model eval]: {opt.eval}")
            print(f"processing ({i:04d})-th image... {img_path}")
        save_images(webpage, visuals, img_path, aspect_ratio=opt.aspect_ratio, width=opt.display_winsize)

        #if i == 0:
        #    export_model = model.netG
        #    export_net = export_model.cpu()
        #    export_net.eval()
##
        #    # 输入张量保持在 CPU
        #    input_tensor = data['A'][:1]
        #    torch.onnx.export(export_model, input_tensor, "spine_model.onnx", verbose=True,
