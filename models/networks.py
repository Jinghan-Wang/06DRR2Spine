...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        # # 论文建议：将输入 input (kV原图) 的高频信息直接保留
        # # 简单有效的做法是将输入与输出进行残差连接 (如果尺寸一致)
        # """
        # 核心修改：将输入 input (kV原图) 与 网络输出进行残差结合
        # 论文逻辑：网络学习的是“改变量”或“分量”，而不是从零生成
        # """
        # # 如果你做的是脊柱分离(提取)，通常是 input - model(input) = background
        # # 或者直接让 model 学习脊柱分量：
        # out = self.model(input)
##
        # # 物理一致性：由于脊柱是 kV 图的一部分，我们让输出与输入做 ResNet 风格的加和
        # # 这样能极大程度防止脊柱在生成过程中由于卷积平移导致的“位置漂移”
        # return out + input
...
class DualHeadUnetGenerator(nn.Module):
    """Create a U-Net generator with a shared trunk and separate image/mask heads."""

    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(DualHeadUnetGenerator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.down = nn.Conv2d(input_nc, ngf, kernel_size=4, stride=2, padding=1, bias=use_bias)
        self.shared_submodule = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, norm_layer=norm_layer)
    ...
    def forward(self, input):
        """Return an image prediction and raw mask logits."""
        shared_input = self.down(input)
        shared_features = self.shared_submodule(shared_input)
        fake_image = self.image_head(shared_features)
        fake_mask_logits = self.mask_head(shared_features)
        return fake_image, fake_mask_logits
